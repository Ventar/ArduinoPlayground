...
CircuitClient::CircuitClient(String host, uint16_t port, String username, String password) {
    debug("[Circuit] created client for username ::= [%s], host ::= [%s], port ::= [%d]", username.c_str(), host.c_str(), port);
...
void CircuitClient::handleWSResponse(DynamicJsonDocument& message) {
    ...
    if (it != _openResponses.end()) {
        ...
        case OpenResponse::CREATE_TEXT_ITEM:
            it->second.conversationAddTextItemCB(ResultCode(ResultCode::OK), TextItem::fromConversationAddTextItemResponse(message));
            break;
        case OpenResponse::SUBSCRIBE_PRESENCE:
            it->second.userSubscribePresenceCB(ResultCode(ResultCode::OK), PresenceState::fromUserSubscribePresenceResponse(message));
            break;
        }
...
void CircuitClient::handleWSEvent(DynamicJsonDocument& message) {
    ...
    if (message["event"]["type"] == "CONVERSATION" &&                             // we have a conversation event here
        message["event"]["conversation"]["type"] == "ADD_ITEM" &&                 // of type ADD_ITEM which is fired whenever a new item was created
        message["event"]["conversation"]["addItem"]["item"]["type"] == "TEXT" &&  // and finally this item is of type TEXT (and not SYSTEM for example)
...
void CircuitClient::setup() {
    ...
    // established.
    _webSocket.onEvent([this](WStype_t type, uint8_t* payload, size_t length) { this->onWebSocketEvent(type, payload, length); });
...
void CircuitClient::createAccessServerCookie() {
    ...
    if (_proxyHost != "") {
        ...
        httpsClient.print(String("") + "POST https://" + _host + ":" + _port +
                          "/sdklogin HTTP/1.1\r\n"
                          "Host: " +
                          _host +
                          "\r\n"
...
void CircuitClient::post(String conversationId, String content, String subject, ConversationAddTextItemCB cb) {
    ...
    String message = TextItem::buildConversationAddTextItemRequest(_requestCounter, conversationId, "", content, subject);
    if (cb != NULL) {
...
void CircuitClient::onConversationAddItemEvent(ConversationAddItemEventCB callback) { this->_conversationAddItemEventCB = callback; }

void CircuitClient::onUserUserPresenceChangedEvent(UserUserPresenceChangedEventCB callback) { this->_userUserPresenceChangedEventCB = callback; }
